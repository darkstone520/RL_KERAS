...
def monitorAccuracy(pltSave=False):
    for cost, color, label in zip(mon_acuuracy_list, mon_color_list[0:len(mon_label_list)], mon_label_list):
        plt.plot(mon_interation_list, cost, c=color, lw=2, ls="--", marker="o", label=label)
    ...
    plt.grid(True)
    if pltSave:
        plt.savefig('Error Graph per Batch {}_{}'.format(CLASS_NUM,time.asctime()))
...
def loadInputData():
    ...
    """
    print("Loading Data")
    with open(__DATA_PATH + "cifar10_data", "r", encoding="utf-8") as file:
...
with tf.Session() as sess:
    ...
    while True:
        ...
        for i in range(total_batch_num):
            iteration_num += 1
            mon_interation_list.append(iteration_num)
            ...
            for m_idx, m in enumerate(models):
                ...
                mon_acuuracy_list[m_idx].append(1.0-a)
