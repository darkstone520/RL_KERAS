...
def two_consecutive_frame_skip(done, env, info, skip_count, real_action):
    ...
    :param env: env
    :param info: 현재 목숨 정보
    :param skip_count: 메인함수의 스킵 카운트가 1, 4, 7 일때만 이 함수가 작동하여 2개의 프레임을 생략한다.
    :return: 각각의 상황에 따라 리턴 값이 달라짐
    """
    if skip_count % 3 == 1 :
        skip_count += 1
        if done == False  :
            ...
            return False, info, skip_count
        return True, 0, skip_count
    else:
        return done, info, skip_count
    ...
    with tf.Session() as sess:
        ...
        for episode in range(1,MAX_EPISODES):
            ...
            while not done:
                ...
                skip_count += 1
                done, info, skip_count = \
                    two_consecutive_frame_skip(done, env, info, skip_count, real_action)
