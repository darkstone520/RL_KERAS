...
EXPLORATION = 750000            # Epsilon 시작부터 마지막 값까지 몇개의 step(time_step, frame, action)으로 줄여나갈 지 정하는 변수
...
def pre_processing(observe):
    ...
    :param observe: env 환경의 state
    :return: 원하는 부분만 cropped 된 (84,84) shape의 게임 화면 이미지
    """
...
def crop_image(image, height_range=(35, 195), width_range=None):
    """ 이미지 상하좌우를 crop 하는 함수

    Args:
        image (3-D array): Numpy image (H, W, C)
        height_range (tuple): Height range between (min_height, max_height)
        width_range (tuple): Width range

    Returns:
        image (3-D array): Numpy image
    """
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        for episode in range(1,MAX_EPISODES):
            ...
            # 랜덤한 일정 시간동안 멈춰있다가(step=1) 움직일 수 있도록 설정하여 잘못된 학습을 미연의 방지
